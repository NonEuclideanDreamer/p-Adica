...
public class Instructions 
{
	...
	public Tile[][] rectangle(int[]c,int[]m,int[]r)
	{
		length=new int[] {3};
		l=3;
		double[][]centers= {{0,0},{-2/Math.sqrt(3),0},{2/Math.sqrt(3),0}};
		double[]ro= {0.5,0.5,0.5};
		Tile[]tile=new Tile[3];
		for(int i=0;i<3;i++)
			tile[i]=new Tile(proto[0],c[i],m[i]==1, new Rn(centers[i]),Math.PI*(0.5+r[i]),1/Math.sqrt(3));
		return new Tile[][] {tile};
	}
	public Tile[][] sierpinski(int[]c, int[]m, int[]r)
	{
		length=new int[] {4};
		l=4;
		double[][]centers= {{0,0},{-1/Math.sqrt(3),0},{0.5/Math.sqrt(3),0.5},{0.5/Math.sqrt(3),-0.5}};
		double[]ro= {1,0,0,0};
		Tile[]tile=new Tile[4];
		for(int i=0;i<4;i++)
			tile[i]=new Tile(proto[0],c[i],m[i]==1, new Rn(centers[i]),Math.PI*(ro[i]+r[i]/3.0),0.5);
		return new Tile[][] {tile};
	}
	public Tile[][] halfIsoc(int[]c, int[]m, int[]r)
	{
		double sqrt=Math.sqrt(3);
		length=new int[] {3};
		l=3;
		double[][]centers= {{sqrt,1},{sqrt,1},{0,0}};
		double[]ro= {5.0/6,0.5,-1.0/6};
		boolean[] mir= {false,true,true};
		Tile[]tile=new Tile[3];
		for(int i=0;i<3;i++)
			tile[i]=new Tile(proto[0],c[i],mir[i], new Rn(centers[i]),Math.PI*(ro[i]+r[i]/3.0),1/sqrt);
		return new Tile[][] {tile};
	}
}
