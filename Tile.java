...
public class Tile 
{
	...
	public void draw(BufferedImage image, double[]min,double[]max, int color)//to be reconsidered for concave tiles
	{
		int c=color;
		int[] one=corners[0].toGrid(min,max,image.getWidth(),image.getHeight()),two,three;
		for (int i=1;i<corners.length-1;i++)
		{
			...
			drawTriangle(one,two,three,image,c,1,nondeg);
